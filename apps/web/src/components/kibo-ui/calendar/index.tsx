...
export const CalendarBody = ({
  ...
  for (let i = 0; i < firstDay; i++) {
    ...
    if (day) {
      days.push(
        showOutsideDays ? (
          <OutOfBoundsDay day={day} key={`prev-${i}`} />
        ) : (
          <div className="h-full w-full" key={`prev-${i}`} />
        )
      );
    }
  }
  ...
  if (remainingDays > 0) {
    for (let i = 0; i < remainingDays; i++) {
      ...
      if (day) {
        days.push(
          showOutsideDays ? (
            <OutOfBoundsDay day={day} key={`next-${i}`} />
          ) : (
            <div className="h-full w-full" key={`next-${i}`} />
          )
        );
      }
    }
  ...
  const weekCount = Math.max(1, totalCells / 7);

  return (
    <div
      className={cn("grid min-h-0 flex-1 grid-cols-7", className)}
      style={{ gridTemplateRows: `repeat(${weekCount}, minmax(0, 1fr))` }}
    >
      {days.map((day, index) => (
        <div
          className={cn(
            "relative min-h-0 overflow-hidden border-t border-r",
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
  ...
  return (
    <div className={cn("grid grid-cols-7 border-t border-border/70 bg-muted/20", className)}>
      {daysData.map((day) => (
        <div className="px-3 py-2 text-center text-muted-foreground text-xs" key={day}>
          {day}
        </div>
      ))}
    </div>
